...
static void *liballoc_memset(void *s, int c, u64 n)
{
    unsigned int i;
    for (i = 0; i < n; i++)
        ((char *)s)[i] = c;
...
static void *liballoc_memcpy(void *s1, const void *s2, u64 n)
{
    char *cdest;
    char *csrc;
...
void liballoc_dump()
{
    ...
    term_print("liballoc: ------ Memory data ---------------\n");
    term_print("liballoc: System memory allocated: %i bytes\n", l_allocated);
    term_print("liballoc: Memory in used (malloc'ed): %i bytes\n", l_inuse);
    term_print("liballoc: Warning count: %i\n", l_warningCount);
    term_print("liballoc: Error count: %i\n", l_errorCount);
    term_print("liballoc: Possible overruns: %i\n", l_possibleOverruns);

    while (maj != NULL)
    {
        term_print("liballoc: %x: total = %i, used = %i\n", maj, maj->size,
                   maj->usage);

        min = maj->first;
        while (min != NULL)
        {
            term_print("liballoc:    %x: %i bytes\n", min, min->size);
            min = min->next;
        }
...
static struct liballoc_major *allocate_new_page(unsigned int size)
{
    ...
    if (maj == NULL)
    {
        l_warningCount += 1;
#if defined DEBUG || defined INFO
        term_print("liballoc: WARNING: liballoc_alloc( %i ) return NULL\n", st);
        FLUSH();
#endif
...
void *PREFIX(malloc)(u64 req_size)
{
    int startedBet = 0;
    ...
    if (size == 0)
    {
        l_warningCount += 1;
#if defined DEBUG || defined INFO
        term_print("liballoc: WARNING: alloc( 0 ) called from %x\n",
...
#if defined DEBUG || defined INFO
#    ifdef DEBUG
        term_print("liballoc: initialization of liballoc " VERSION "\n");
#    endif
        ...
        if (l_memRoot == NULL)
        {
            liballoc_unlock();
#ifdef DEBUG
            term_print("liballoc: initial l_memRoot initialization failed\n",
                       p);
            FLUSH();
#endif
            return NULL;
...
#ifdef DEBUG
        term_print("liballoc: set up first memory major %x\n", l_memRoot);
        FLUSH();
#endif
    }
    ...
    while (maj != NULL)
    {
        ...
        {
#    ifdef DEBUG
            term_print("CASE 1: Insufficient space in block %x\n", maj);
            FLUSH();
#    endif
...
#    ifdef DEBUG
                    term_print("CASE 4.1: returning %x\n", p);
                    FLUSH();
#    endif
                    liballoc_unlock(); // release the lock
                    return p;
...
#    ifdef DEBUG
                    term_print("CASE 4.2: returning %x\n", p);
                    FLUSH();
#    endif
...
#ifdef DEBUG
    term_print("All cases exhausted. No memory available.\n");
    FLUSH();
#endif
#if defined DEBUG || defined INFO
    term_print("liballoc: WARNING: PREFIX(malloc)( %i ) returning NULL.\n",
               size);
    liballoc_dump();
    FLUSH();
#endif
...
void PREFIX(free)(void *ptr)
{
    ...
    if (ptr == NULL)
    {
        l_warningCount += 1;
#if defined DEBUG || defined INFO
        term_print("liballoc: WARNING: PREFIX(free)( NULL ) called from %x\n",
                   __builtin_return_address(0));
        FLUSH();
#endif
        return;
...
void *PREFIX(calloc)(u64 nobj, u64 size)
{
    int real_size;
    void *p;
...
void *PREFIX(realloc)(void *p, u64 size)
{
    void *ptr;
    struct liballoc_minor *min;
