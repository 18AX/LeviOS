...
static void print_pad(char **at, u32 *left, u64 *ret, char p, int num)
{
    while (num--)
    {
        if (*left > 1)
...
static void spool_str(char **at, u32 *left, u64 *ret, const char *buf, int len)
{
    int i;
    for (i = 0; i < len; i++)
    {
...
{
    u64 ret = write(STDOUT_FD, str, strlen(str));
